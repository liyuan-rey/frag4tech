...
实际是建立外键约束还是在代码访问时遵循外键逻辑，根据对数据完整性的要求决定。
...
- NULL 值
  ...
  数据库中不建议出现可为空NULL字段。应该在 SQL 建表脚本中明确指明使用缺省。
  由于 NULL 值在参加任何运算中，结果均为 NULL，所以在应用程序中必须利用 coalesce() 函数
  把可能为 NULL值的字段或变量转换为非 NULL 的默认值。例如：coalesce(item_num, 0)。
